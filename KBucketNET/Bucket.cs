...
namespace KBucketNET
{
    ...
    ///   A binary tree node.
    /// </summary>
    public sealed class Bucket<T> where T : class, IContact
    {
        /// <summary>
        ...
        {
            return Contacts != null && Contacts.Any(c => c.Id.SequenceEqual(item.Id));
        }
        ...
        /// </returns>
        public T Get(IEnumerable<byte> id)
        {
            return Contacts?.FirstOrDefault(c => c.Id.SequenceEqual(id));
        }

        internal int IndexOf(IEnumerable<byte> id)
        {
            if (Contacts == null)
            {
                return -1;
